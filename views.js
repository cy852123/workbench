...
/* 收集箱条目的 type 存的是代码（link/text/file），界面要显示中文 —— 原来直接把代码印出来了 */
  ...
  function kaoyanOverview(dm) {
    ...
    html += card(cardHead("今日日程", doneCount + " / " + cards.length + " 完成", "tasks"),
  ...
  function kyMistakeFiles() {
    ...
    html += card(cardHead("批量上传", "支持 PDF / 图片 / 压缩包", "upload"),
    ...
    html += card(cardHead("已上传材料", mf.length + " 份", "files"),
    ...
    html += card(cardHead("知识点总结", kn.length + " 条", "knowledge"),
  ...
  function kyEnglishPage(dm) {
    ...
    html += card(cardHead("作文模板库", "大作文 / 小作文 分类上传", "essay"),
  ...
  function kyMathPage(dm) {
    ...
    html += card(cardHead("粗心账本", cl.length ? "共 " + cl.length + " 次失误" : "暂无记录", "careless") +
  ...
  function zoneWriting(dm) {
    return card(cardHead("作文批改", "已改用微信 AI", "ai-writing"),
  ...
  function zoneSpeaking(dm) {
    return card(cardHead("口语练习", "已改用微信 AI", "ai-speaking"),
  ...
  function mistakes() {
    ...
    html += '<div class="grid grid-2">' +
      card(cardHead("错题统计", "今日待复习优先", "mk-stat"),
        ...
        "</div>") +
      card(cardHead("快速记录", "错题是复习的宝藏", "mk-add"),
  ...
  function qa() {
    ...
    html += '<div class="grid grid-2">' +
      ...
      card(cardHead("记录问题", "不懂就问，弄懂就记", "qa-add"),
  ...
  function settings() {
    ...
    html += card(cardHead("更新日志", "每次更新都会记录在这里", "changelog"),
      '<div class="log-item"><div class="log-date">2026-09-18 · 删掉词典 + 云同步失败会提醒<span class="log-tag">清理</span></div>' +
      '<p>词典相关的东西整条删干净：词库原料（68 MB）、词典脚本，还有弹窗里那句「缺释义的已自动从内置词库补充」—— 那句话其实从来没生效过，留着只会骗人。查词功能本来也没接进界面，之后不再做（手机上用翻译软件更方便）。</p>' +
      '<p>另外修了一个看不见的坑：<b>自动上传云端失败原来悄无声息</b>（你以为云上是最新的，其实可能停在几天前）。现在失败会在左下角一直显示「⚠️ 云端未同步 时:分（本地已保存）」，下次同步成功自动恢复成「已保存 · 云端已同步」。</p>' +
      '<p>影响范围：帮助文案、云同步提示。数据：无影响，一个都没删。你需要的操作：无（手机上把 App 划掉重开一次即新版）。</p></div>' +
